...
export type AdminPanelInitialTarget = {
  tab: 'reports' | 'moderation' | 'bug_reports'
  targetType?: AdminReportTargetType | 'bug_report'
  targetId: number
}
...
function AdminPanelDialog({ open, authToken, currentUserRole, initialTarget = null, onNavigate, onClose }: AdminPanelDialogProps) {
  ...
  const loadReports = useCallback(async () => {
    ...
    try {
      ...
      setSelectedReportKey((previous) => {
        const requestedKey =
          initialTarget?.tab === 'reports' && initialTarget.targetType && initialTarget.targetType !== 'bug_report'
            ? `${initialTarget.targetType}:${initialTarget.targetId}`
            : null
        if (requestedKey && response.some((report) => buildReportKey(report) === requestedKey)) {
          return requestedKey
        }
  ...
  }, [authToken, canUseAdminPanel, initialTarget, open, resetSelectedReportTargetPayloads])
  ...
  const loadBugReports = useCallback(async () => {
    ...
    try {
      ...
      setSelectedBugReportId((previous) => {
        if (
          initialTarget?.tab === 'bug_reports' &&
          initialTarget.targetId > 0 &&
          response.some((report) => report.id === initialTarget.targetId)
        ) {
          return initialTarget.targetId
        }
      ...
      } else if (initialTarget?.tab === 'bug_reports' && response.some((report) => report.id === initialTarget.targetId)) {
  ...
  }, [authToken, canUseAdminPanel, initialTarget, open])
  ...
  const loadModerationQueue = useCallback(async () => {
    ...
    try {
      ...
      setSelectedModerationKey((previous) => {
        const requestedKey =
          initialTarget?.tab === 'moderation' && initialTarget.targetType && initialTarget.targetType !== 'bug_report'
            ? `${initialTarget.targetType}:${initialTarget.targetId}`
            : null
        if (requestedKey && items.some((item) => buildModerationKey(item) === requestedKey)) {
          return requestedKey
        }
  ...
  }, [authToken, canUseAdminPanel, initialTarget, open, resetModerationDrafts])
                ...
                >
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 1.4 }}>
                    <Box sx={adminActionCardSx}>
                      <Typography sx={adminActionLabelSx}>Солы</Typography>
