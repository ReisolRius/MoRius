...
function ShopPage({ user, authToken, onNavigate, onUserUpdate }: ShopPageProps) {
  ...
  const renderSubscriptionCard = (plan: SubscriptionPlan, index: number) => {
    ...
    const bullets =
      (SUBSCRIPTION_PACKAGE_BULLETS as Record<string, readonly string[] | undefined>)[plan.id] ?? plan.perks
    return (
      <SubscriptionPlanCard
        key={plan.id}
        title={plan.title}
        priceLabel={formatPrice(plan.price_rub)}
        bullets={bullets}
        styleIndex={index}
        buyLabel={isLocked ? 'Скоро добавим' : 'Купить'}
        onBuy={() => handleOpenSubscribe(plan)}
        disabled={isLocked}
        dimmed={isLocked}
      />
    )
  ...
  const renderPlanCard = (plan: CoinTopUpPlan, index: number) => {
    ...
    const bullets =
      (COIN_PACKAGE_BULLETS as Record<string, readonly string[] | undefined>)[plan.id] ??
    ...
    return (
      <CoinPackageCard
        ...
        title={plan.title}
        priceLabel={formatPrice(plan.price_rub)}
        coinsLabel={String(Math.max(0, Math.trunc(plan.coins)))}
        bullets={bullets}
        styleIndex={index}
        buyLabel={isPaying ? 'Открываем оплату...' : 'Купить'}
        onBuy={() => void handleBuyPlan(plan)}
        disabled={isPaying}
      />
    )
  }
