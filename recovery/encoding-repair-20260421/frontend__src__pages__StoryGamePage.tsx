...
const ASSISTANT_SPEAKER_NAME_TOKEN_PATTERN = /^[0-9A-Za-z\u0410-\u042f\u0430-\u044f\u0401\u0451'РІР‚в„ў-]+$/u
const GENERIC_DIALOGUE_SPEAKER_DEFAULT = 'Р СњР СџР РЋ'
...
function normalizeAssistantMarkerKey(value: string): string {
  const normalizedKey = value.toLowerCase().replace(/[\\s-]+/g, '_').replace(/РЎ‘/g, 'Р Вµ').trim()
  const compactKey = normalizedKey.replace(/_/g, '')
  const aliasKeyByCompact: Record<string, string> = {
  const normalizedKey = value.toLowerCase().replace(/[\\s-]+/g, '_').replace(/РЎвЂ/g, 'Р Вµ').trim()
    narrator: 'narrator',
    narrative: 'narrative',
    '\u0440\u0430\u0441\u0441\u043a\u0430\u0437\u0447\u0438\u043a': 'narrator',
    '\u043d\u0430\u0440\u0440\u0430\u0442\u043e\u0440': 'narrator',
...
function resolveAssistantTagKind(markerKey: string): AssistantTagDescriptor | null {
  ...
  if (
    ...
    return {
      kind: 'speech',
      defaultSpeakerName: 'Р “Р “',
    }
  }
      defaultSpeakerName: 'Р вЂњР вЂњ',
...
const ENVIRONMENT_MONTH_OPTIONS: ReadonlyArray<{ value: string; label: string; season: EnvironmentSeasonValue }> = [
  ...
  { value: '5', label: 'Р СљР В°Р №', season: 'spring' },
  { value: '6', label: 'Июнь', season: 'summer' },
  { value: '7', label: 'Июль', season: 'summer' },
  { value: '5', label: 'Р СљР В°Р в„–', season: 'spring' },
...
const ENVIRONMENT_SEASON_OPTIONS: ReadonlyArray<{ value: EnvironmentSeasonValue; label: string }> = [
  { value: 'winter', label: 'Р —Р С‘Р СР В°' },
]
...
const ENVIRONMENT_SEASON_OPTIONS: ReadonlyArray<{ value: EnvironmentSeasonValue; label: string }> = [
  ...
  { value: 'winter', label: 'Р вЂ”Р С‘Р СР В°' },
...
function resolveEnvironmentSeasonValueFromLabel(value: unknown): EnvironmentSeasonValue | null {
  ...
  if (normalized.includes('Р В·Р С‘Р С') || normalized.includes('winter')) {
    return 'winter'
  }
  ...
  if (normalized.includes('Р В»Р ВµРЎвЂљ') || normalized.includes('summer')) {
  return null
...
function resolveEnvironmentSummaryIcon(summaryValue: unknown): string {
  ...
  if (
    summary.includes('Р Т‘Р С•Р В¶Р Т‘') ||
    summary.includes('Р В»Р С‘Р Р†') ||
    summary.includes('гроз') ||
    summary.includes('rain') ||
    summary.includes('storm')
  ) {
...
function formatEnvironmentDisplayInfo(
  ...
  if (!parsed) {
  const seasonAndMonth = [seasonLabel, monthLabel].filter(Boolean).join(' РІР‚Сћ ')
    return {
        ...
        >
            ...
            <Box data-tour-id="story-world-cards-panel" sx={{ display: 'flex', flexDirection: 'column', gap: 1.1, minHeight: 0, flex: 1 }}>
              ...
              >
                {[
                  ...
                  { key: 'world' as const, label: 'Р СљР С‘РЎР‚', iconMarkup: cardsWorldTabIconMarkup },
                ].map((tab) => {
                  return (
                    <Button
                      key={tab.key}
          ...
          {!shouldShowRightPanelLoadingSkeleton && rightPanelMode === 'world' && activeWorldPanelTab === 'world' ? (() => {
            ...
            const environmentWeatherMeta = environmentWeatherEnabled
              ? [environmentDateInfo.title, environmentDateInfo.meta].filter(Boolean).join(' РІР‚Сћ ')
            ...
            const environmentHeaderMeta = environmentTimeEnabled
              ...
              ? [environmentDateInfo.seasonAndMonth ?? environmentDateInfo.season, environmentSummary].filter(Boolean).join(' РІР‚Сћ ')
                    ...
                    <Collapse in={isFineTuneSettingsExpanded} timeout={200} unmountOnExit>
                      <Box data-tour-id="story-settings-finetune-panel" sx={{ pb: 0.9, pt: 0.08 }}>
                        ...
                        >
                          ...
                          <Box sx={{ mt: 0.34 }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="center">
                              <Typography sx={{ color: 'var(--morius-text-primary)', fontSize: '0.8rem', fontWeight: 700 }}>Р СћР С•Р С—-K</Typography>
                          ...
                          <Box sx={{ mt: 0.34 }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="center">
                              <Typography sx={{ color: 'var(--morius-text-primary)', fontSize: '0.8rem', fontWeight: 700 }}>Р СћР С•Р С—-P</Typography>
          ...
          >
            ...
            >
              <Stack direction="row" alignItems="flex-end" spacing={0.82}>
                ...
                <Tooltip arrow placement="top" title="Р СњР В°Р В·Р В°Р Т‘">
                  <span>
                    <IconButton
              ...
              {editingWorldCardKind === 'npc' ? (
                <Stack spacing={0.35}>
                  ...
                  >
                    ...
                    <option value="3">3 РЎ…Р С•Р Т‘Р В°</option>
                    <option value="5">5 ходов</option>
                    <option value="10">10 ходов</option>
                    <option value="3">3 РЎвЂ¦Р С•Р Т‘Р В°</option>
          ...
          {false ? (
            ...
            <Stack spacing={0.35}>
              ...
              >
                ...
                <option value="2">2 РЎ…Р С•Р Т‘Р В°</option>
                <option value="3">3 РЎ…Р С•Р Т‘Р В°</option>
                <option value="5">5 ходов</option>
                <option value="2">2 РЎвЂ¦Р С•Р Т‘Р В°</option>
                <option value="10">10 ходов</option>
                <option value="3">3 РЎвЂ¦Р С•Р Т‘Р В°</option>
