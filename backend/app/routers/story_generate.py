...
def _fallback_build_provider_messages(
    *,
    ...
    system_prompt = (
        ...
        "- Для говорящего из карточек всегда используй точный title карточки как Name.\n"
        "- Новому NPC до первой реплики придумай естественное устойчивое имя и затем используй его без изменений.\n"
        "- Только если личность пока должна быть скрыта, вместо имени используй конкретное устойчивое обозначение роли или положения длиной не более 4 слов; после раскрытия перейди на точное имя.\n"
        "- Имена и обозначения в [[NPC:...]] из предыдущих сообщений неизменны: для того же персонажа в следующих ходах используй их точно.\n"
        "- Каждая прямая речь NPC, включая единичный возглас, шёпот, реплику из толпы и речь за кадром, всегда начинается с [[NPC:...]].\n"
        "- Запрещены общие подписи НПС, NPC, Голос, Незнакомец и Персонаж; выбери имя или конкретную роль.\n"
