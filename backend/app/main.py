...
STORY_FORCED_OUTPUT_TRANSLATION_MODEL_BY_STORY_MODEL: dict[str, str] = {
    ...
    "moonshotai/kimi-k2.5": STORY_SERVICE_TEXT_MODEL,
...
STORY_POLZA_PROVIDER_PINNED_BY_MODEL = {
    ...
    "moonshotai/kimi-k2.5": STORY_POLZA_PROVIDER_ROUTERAI,
...
def _build_story_system_prompt(
    ...
    *,
    ...
    visual_novel_contract_active = any(
        str(card.get("source_kind", "") or "").strip().lower() == "visual_novel"
        for card in instruction_cards_for_prompt
    )
    ...
    if visual_novel_contract_active:
        final_check_lines.extend(
            [
                "В режиме визуальной новеллы каждый без исключения абзац заканчивается ровно одним корректным {{VN_CAST|...}}; это обязательное системное исключение не удаляй и не заменяй.",
                "Каждый новый или непрописанный персонаж, который видим, упомянут как участник текущего кадра, говорит или думает, присутствует в VN_CAST под тем же устойчивым именем и с полом male/female; отсутствие карточки не является причиной скрывать его из состава.",
            ]
        )
    final_check_lines.extend(
        [
            ...
            (
                "В ответе только живая русская проза сцены — без рассуждений, черновиков и посторонних служебных пометок."
                if visual_novel_contract_active
                else "В ответе только живая русская проза сцены — без рассуждений, черновиков и служебных пометок."
            ),
        ]
    )
...
def _select_story_temperature_value(
    *,
    ...
    if not _can_apply_story_sampling_to_model(model_name):
        return None
    if _normalize_story_model_id(model_name) in STORY_TEMPERATURE_DISABLED_MODEL_IDS:
        return None
