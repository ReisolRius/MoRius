...
def _ensure_story_game_community_columns_exist(private_visibility: str, default_response_max_tokens: int) -> None:
    ...
    if "story_reasoning_enabled" not in existing_columns:
        alter_statements.append(
            f"ALTER TABLE {StoryGame.__tablename__} "
            "ADD COLUMN story_reasoning_enabled BOOLEAN NOT NULL DEFAULT 0"
        )
    if "image_model" not in existing_columns:
        alter_statements.append(
            f"ALTER TABLE {StoryGame.__tablename__} "
            "ADD COLUMN image_model VARCHAR(120) NOT NULL DEFAULT 'google/gemini-2.5-flash-image'"
        )
    ...
    if "active_main_hero_card_id" not in existing_columns:
        alter_statements.append(
            f"ALTER TABLE {StoryGame.__tablename__} "
            "ADD COLUMN active_main_hero_card_id INTEGER"
        )
    if "auto_npc_cards_enabled" not in existing_columns:
        alter_statements.append(
            f"ALTER TABLE {StoryGame.__tablename__} "
